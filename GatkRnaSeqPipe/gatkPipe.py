...
class GATKPipe:
    ...
    def filter_vars(invcf,di,rid,ref):
        ...
        comm14 = f"gatk VariantFiltration -R {ref} -V {invcf} -O {filt_vcf} --filter-name qual_filt --filter-expression \"QUAL > 25.0\" --filter-name qual_depth_filt --filter-expression \"QD > 20.0\" --filter-name depth_filt --filter-expression \"DP > 5\""
