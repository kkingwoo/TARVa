...
The following steps are executed for the pipeline:

## Sequencing read QC and alignment

1.  [**Mono_MCI.slurm**](Mono_MCI.slurm) is the main slurm script for
    ...
    - [**match_monocytes.py**](match_monocytes.py)<a name="match-monocytes"></a>
      ...
      2.  creates and submits a slurm script to the job scheduler, which
          ...
          parallel for samples and imports [**star_run.py**](#star-run)
          as a custom module which executes STAR alignment for each
          sample.

## Alignment QC and variant-finding

2.  [**full_gatk.slurm**](full_gatk.slurm) is the main slurm script for
    qc of alignments and variant-finding. Runs
    [**full_gatk.py**](#full-gatk)
    - [**full_gatk.py**](full_gatk.py)<a name="full-gatk"></a> runs the
      GATK pipeline in parallel and imports [**gatkPipe.py**]() as a
      custom module which contains commands for each step of the
      pipeline.
